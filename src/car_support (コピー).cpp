...
#define WinSize_default {1920,1080}
#define cam_Img_Size_Width 640
#define cam_Img_Size_Height 480
...
cv::Size2i WinSize=WinSize_default;
...
cv::Mat create(int display,cv::Mat Img)
{
	...
	pos={cam_Img_Size_Width*display/3,distance_size};

	dtmp=WinSize.width*3/cam_Img_Size_Width;
	dtmp=WinSize.height/dtmp;
		
	cv::Mat cut_img(Img,cv::Rect(pos.x,pos.y,cam_Img_Size_Width/3,(int)dtmp));
	cv::resize(cut_img,cut_img,WinSize);

	if(display==1){
		cut_img=shift_view(shift,cut_img);
		cut_img=speed_view(odom_liner_x,cut_img);
	}

	flip(cut_img, cut_img, 0);								//Opencvの画像をOpenGLに合うように座標軸を変更
	cvtColor(cut_img, cut_img, cv::COLOR_BGR2RGB);				//Opencvの画像をOpenGLに合うように画像色を変更
